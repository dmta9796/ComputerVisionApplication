...
class meanshiftsaturation():
    ...
    def start(self):
        ...
        sat_roi = hsv_roi[:,:,1]
        #mask = cv2.inRange(sat_roi, np.array((lowerhue, lowersat,lowervalue)), np.array((upperhue,uppersat,uppervalue)))
        mask = cv2.inRange(sat_roi, np.array((lowersat)), np.array((uppersat)))
        self.roi_hist = cv2.calcHist([sat_roi],[0],mask,[upperhue],[lowerhue,upperhue])
    ...
    def step(self,step):
        ...
        if(ret):
            ...
            sat = hsv[:,:,1]
            dst = cv2.calcBackProject([sat],[0],self.roi_hist,[0,180],self.binstep)
